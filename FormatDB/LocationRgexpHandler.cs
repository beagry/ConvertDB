...
namespace Formater
{
    internal class LocatonRegexpHandler
    {
        ...
        private readonly Regex settlemenToLeftRegex =
            new Regex(@"(?n)\(?\""?(?<type>(?i)\b(с(\/|\\)?с|с(\/|\\)?(п)|с(\/|\\)о)\b)\s*(?<name>(\b(?!\d)\w[^\,\\\/\(\)\b\s]{2,}\b\s?){1,2})\)?($|\.|\""""|\,|\s|\)|\()+", RegexOptions.Compiled);
        ...
        private readonly Regex streetRegex =
            new Regex(
                @"(?n)(^|\.|\""|\,|\s|\)|\()+\""?(?<name>(\d+\-?(\w*?я|\w*?й)\s?)?\b(?<!((пос)\.)\s)(?!(?i)коттеджн|имеет|Район|до\b|есть|зимой|от\b|круглый\sгод|за\b|\d+км|проход|как\b|так\b|по\b|при\b|ул\b|провед|км\b|без\b|ведут|газ\b|для|дач\b|дом\b|круглый\sгод|этажн|сел(а|о)|посел|дом\b|до\b|вы\b|цена|или\b|деревни|напротив|участ|водопров|сеть|коммуникац|канализ)(\w|\s){3,}(\d+\-?(\w*?я|\w*?й)\s?)?(?<trakt>\sтракт)?)\s(?<type>(?i)\bпр(оезд|д)?\b|\bш(оссе)?\b|\bтракт\b|\bпер(\.?)\b|\b(ул(и(ц(а|(?<=(по|на)\s\[А-Я]w+\sулиц)е)?)?)?|(?<=(ий|ый)\s)п(?=\w)((р(ос(пе|\.|\-)\.|\-)?|\-)?к?(?(\.)т?|т))|пер|ми?(кр)?\-?(орай)?о?н?\.?\))\b\.?)(\.|\,|\s|\))*((д(ом)?\.?\s*)?(?<house_num>\d(\d|\-|\\|\/){0,4})(?<letter>\w+)?)*($|\.|\""|\,|\s|\)|\()*",
                RegexOptions.Compiled);

        private readonly Regex streetToLeftRegex =
            new Regex(
                @"(?n)(^|\-|\.|\""|\,|\s|\)|\()+(?<!(?i)(рядом(\sс)?|окколо|двух)\s)(?<type>(?i)\bпр(оезд|д)?\b|\bтракт\b|\b(ул(и(ц(а|(?<=(по|на|в)\sулиц)е|е(?=\s(?-i)[А-Яа-я])|ы)?)?|\.(?((?=\s?\w+[А-Яа-я])|(?!\w+\b\s?(&|\,|\.|\(|\))))\s?|\s)|\s\.(?=[А-Яа-я][А-Яа-я]+))?(?=\s?\.?\s*\w+)|п(?=\w)((р(ос(пе)?)?|)?\-?к?т)|пер(еулок|\.)?|м(?=\w)(и(?!н))?(кр)?\-?(орай)?о?(н([А-Яа-я]{0,2}))?(?!\s\w+\sул\b)|б(ульва|\-)р|тупик|(про|разъ|въ)езд(?!\s(авто|на\b|в\b|вдоль\b))|ш(оссе(?!\s\d)|\.)?|алл(ея|\.))\)?\b)(\s|\""|'|`)*(?<name>(\d+\sлет\s)?\b(?!((?i)коттеджн|пролож|проход|тянетс|имеет|Район|до\b|есть|зимой|от\b|круглый\sгод|за\b|\d+км|круглый\sгод|проход|как\b|так\b|по\b|при\b|ул\b|провед|км\b|без\b|ведут|газ\b|для|дач\b|дом\b|этажн|сел(а|о)|посел|дом\b|до\b|вы|цена|или\b|деревни|напротив|участ|водопров|сеть|коммуникац|канализ)\b)([А-Яа-я][А-Яа-я]{0,3}\.\s?[А-Яа-я][А-Яа-я]+|\d+(?!(\d*?\s?км|\s|$))((\sлет)|(\s?\-?\s?[А-Яа-я]+)?\s?\-?\s?[А-Яа-я]{0,})\b(?=\s+\w+)|[А-Яа-я]{2,}(\s[А-Яа-я\w\-]+\s*$)?|([А-Яа-я]|\s?\-\s?){2,}\d+?)(?(\s[А-Яа-я]\w+\s?(\)|:|-|\.|\,|\""))\s[А-Яа-я]\w+))(\.|\,|:|\s|\)|\()*(((д(ом)?|уч)?\.?)\s*(?<house_num>\b\d(\d|\\|/|\w){0,5}(?!(\s?(\.|\,)\s?\d+)?\s(сот|га)))(?(house_num)(\s?к(орп(ус))?\.?\s?(?<korp>\d+))?))?($|\.|\""|\,|\s|\)|\()+",
                RegexOptions.Compiled);

        private readonly Regex subjRegEx =
            new Regex(
                @"(?n)((?<pre>(?i)(по|в|за))\s)?(?<name>\b[А-Яа-я\-]+\b)\s(автономная(?=\sобл)\s)?(?<type>(?i)(?<=(ая|ой)\s)(обл(\.|\b|(аст(ь|и))))|(?<=(ий|ый)\s)край|респ)",
                RegexOptions.Compiled);
        ...
        private readonly Regex wordWithHeadLetteRegex =
            new Regex(
                @"(?n)(\d(\-?\w{1,2})?\s)?((?<=[^\w\s]\s)[А-Я]\w{0,2}\.)?\b(?<!(от)\s)(?!(?i)(земе?л|продаж))(?!(?#Слова исключения)(?i)участок|ИЖС|М\d+|юг\b|снт\b|село\b|пос\b|км\b|сад\b|ГК\b|до\b|база\b)[А-Я](\w|\-)+\b(?(\s[А-Я]\w+($|\s?\(|\)|""|'))\s\w+)(\s\d\-?\w{1,2})?(?=\s*($|\""|”|“|»|\.|\,|\s|\)|'|`|&quot;|\())(\d\-?\w{1,2}\s)?",
                RegexOptions.Compiled);
...
//        }
        ...
        public Regex SettlemenToLeftRegex
        {
            get { return settlemenToLeftRegex; }
        }
