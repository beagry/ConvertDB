...
namespace Formater
{
    public partial class DbToConvert
    {
        ...
        {
            //bug в поселение записывает ""
            //bug красный субъект при пустые других значениях
// ReSharper disable once UnusedVariable
            var checkedValues = new Dictionary<string, string>();

            const int percentForThisMethod = 70;
            var everyEachStep = lastUsedRow/percentForThisMethod;
            var currStep = 0;

            var per10 = new DateTime();
            var per100 = new DateTime();
            var per500 = new DateTime();
            var per1000 = new DateTime();
                    ...
                    foreach (var regi in regs)
                    {
                        ...
                        if (matches.Count > 0)
                        {
                            ...
                            if ((string.Equals(name, regName, StringComparison.OrdinalIgnoreCase) &&
                                ...
                                (nearCityCell.IsEmpty() == false && (string.Equals(nearCityCell.Value.ToString(), name,
