...
//                var dataRow = new ExcelLocationRow(worksheet,row,wbType);
                ...
                //и затираем ячейки
                var subjValue = subjCell.Value is string ? ReplaceYO(subjCell.Value.ToString()) : string.Empty;
                subjCell.Value = string.Empty;

                var regionValue = regionCell.Value is string
                    ? ReplaceYO(regionCell.Value.ToString())
                    : string.Empty;
                regionCell.Value = string.Empty;

                var nearCityValue = nearCityCell.Value is string
                    ? ReplaceYO(nearCityCell.Value.ToString())
                    : string.Empty;
                nearCityCell.Value = string.Empty;

                var vgtValue = vgtCell.Value is string ? ReplaceYO(vgtCell.Value.ToString()) : string.Empty;
                vgtCell.Value = string.Empty;

                var sourceLinkValue = worksheet.Cells[row, sourceLinkColumn].Value is string
                    ? ReplaceYO(worksheet.Cells[row, sourceLinkColumn].Value.ToString())
                    : string.Empty;

                var landmarkValue = landmarkCell.Value is string
                ...
                //определяем по источнику
                var subjectName = subjectSourceWorksheet.GetSubjectBySourceLink(sourceLinkValue);


                if (string.IsNullOrEmpty(subjectName))
                    subjectName = oktmo.GetFullName(subjValue, OKTMOColumns.Subject);

                //определили
                if (!string.IsNullOrEmpty(subjectName))
                {
                    //вставляем
                    subjCell.Value = subjectName;

                    //отбираем
                    if (oktmo.StringMatchInColumn(customTable, subjectName,
                        OKTMOColumns.Subject))
                    {
                        ...
                        //Get RegCenter
                        regCenter = oktmo.GetDefaultRegCenterFullName(subjectName, ref regName);
                ...
                //наличие инфо о муниципальном образовании
                Regex tmpRegex;
                if (!string.IsNullOrEmpty(regionValue))
                {
                    ...
                    if (match.Success)
                    {
                        ...
                        var fullName = oktmo.GetFullName(TryChangeSubjectEndness(match.Groups["name"].Value),
                            OKTMOColumns.Subject);
...
//                            customTable = subjectTable.Copy();
                    ...
                    if (match.Success)
                    {
                        ...
                        else
                        {
                            ...
                            else if (nearCityCell.IsEmpty() == false) //bug ячейка ещё не проверена
                            {
                    ...
                    if (match.Success)
                    {
                        ...
                        sntKpDnpCell.Value = sntKpDnpCell.IsEmpty() ? newName : ", " + newName;
                        regionValue = tmpRegex.Replace(regionValue, ", ");
                    }
                        ...
                        else
                        {
                            ...
                            if (oktmo.StringMatchInColumn(subjectTable, name, OKTMOColumns.NearCity))
                            {
                                ...
                                if (newTable.Rows.Count == 1)
                                {
                                    ...
                                    if (typeOfNearCity.IsEmpty() || typeOfNearCity.Value.ToString() != newType)
                                    {
                                        type = newType;
                                    }
...
//                                }
                    ...
                    if (match.Success)
                    {
                        ...
                        else
                        {
                            ...
                            else if (nearCityCell.IsEmpty() == false)
                            {
                                nearCityCell.Style.Fill.PatternType = ExcelFillStyle.Solid;
                                nearCityCell.Style.Fill.BackgroundColor.SetColor(ExcelExtensions.BadColor);
                            }
                    ...
                    if (match.Success)
                    {
                        while (match.Success)
                        {
                            ...
                            sntKpDnpCell.Value = sntKpDnpCell.IsEmpty() ? name : ", " + name;
                    ...
                    if (matches.Count > 0)
                    {
                        if (!cellsFilled)
                        {
                            ...
                            if (nearCityCell.IsEmpty() || nearCityCell.Value.ToString() == string.Empty)
                            {
                    ...
                    if (cell.IsEmpty() == false)
                    {
                        ...
                        if (match.Success)
                        {
                            do
                            {
                                ...
                                sntKpDnpCell.Value = sntKpDnpCell.IsEmpty() ||
                        ...
                        if (match.Success)
                        {
                            ...
                            if (!string.IsNullOrEmpty(fullName) &&
                                subjCell.IsEmpty() == false &&
                                !string.Equals(subjCell.Value.ToString().Trim(), fullName.Trim(),
                                    StringComparison.OrdinalIgnoreCase))
                                //subjCell.Value.ToString()
                                //    .IndexOf(match.Groups["name"].Value, StringComparison.OrdinalIgnoreCase) == -1)
                            {
                        ...
                        foreach (var regi in regs)
                        {
                            ...
                            if (!string.IsNullOrEmpty(match.Groups["out"].Value))
                            {
                                ...
                                if (!string.IsNullOrEmpty(match.Groups["dist"].Value))
                                {
                                    ...
                                    if (string.Equals(name, regName, StringComparison.OrdinalIgnoreCase))
                                    {
                                        ...
                                        if (distToNearCityCell.IsEmpty() == false)
                                            landmarkCell.Value +=
                            ...
                            if ((string.Equals(name, regName, StringComparison.OrdinalIgnoreCase) &&
                                ...
                                (nearCityCell.IsEmpty() == false &&
                                 (string.Equals(nearCityCell.Value.ToString(), name,
                                     StringComparison.OrdinalIgnoreCase)))) continue;

                            if (type == "город" && typeOfNearCity.IsEmpty() == false &&
                            ...
                            else
                            {
                                ...
                                //Bug нихрена мы не в любом случае записивыем
                                //ведь текущий нас пункт может подходить, а найденный нет
                                nearCityCell.Value = name;
                                typeOfNearCity.Value = type;
...
//              //Подтираем столбец Ориентир
                ...
                if (nearCityCell.IsEmpty())
                {
                    ...
                    else if (regionCell.IsEmpty() == false &&
                    ...
                    {
                        var name = regionCell.Value.ToString().Replace("город", "");
                ...
                else if (typeOfNearCity.IsEmpty() &&
                         string.Equals(nearCityCell.Value.ToString(), regName, StringComparison.OrdinalIgnoreCase))
                {
                    typeOfNearCity.Value = "город";
                }
                ...
                if (vgtCell.IsEmpty() == false)
                {
                        ...
                        !vgtWorksheet.CombinationExists(nearCityCell.Value.ToString(), vgtCell.Value.ToString()))
                    {
                        ...
                        var cities = vgtWorksheet.GetCitiesListByTerritory(vgtCell.Value.ToString());

                        //Когда населенный пункт пустой, а муниципальное образовать есть
                        if (subjectTable != null && regionCell.IsEmpty() == false && nearCityCell.IsEmpty())
                        {
                            ...
                            !string.Equals(typeOfNearCity.Value.ToString(), "город", StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            {
                                AppendToLandMarkCell(vgtCell.Value + " район", row);
                                vgtCell.Value = string.Empty;
                            }
                        }
        ...
        private void AppendToLandMarkCell(string value, int row)
        {
            ...
            if (landmarkCell.IsEmpty() == false &&
                landmarkCell.Value.ToString().IndexOf(value, StringComparison.Ordinal) >= 0)
                return;


            if (value.IndexOf("район", StringComparison.Ordinal) >= 0)
                if (landmarkCell.IsEmpty())
            ...
            ref bool cellsFilled, string regCenter, string regName)
        {
            ...
            while (match.Success)
            {
                ...
                if (match.Value != (string) regionCell.Value && match.Value != (string) nearCityCell.Value &&
                    (sntKpDnpCell.IsEmpty() ||
                     (sntKpDnpCell.Value.ToString().IndexOf(match.Value,
                         StringComparison.OrdinalIgnoreCase) == -1)))
                {
...
//                            TryFillClassificator(row, customTable, ref cellsFilled);
                    ...
                    else
                    {
                            ...
                            //Строка будет  заполнена, если существует всего один насел пункт с таким районом
                        {
                            ...
                            if (nearCityCell.IsEmpty() == false &&
                                nearCityCell.Style.Fill.BackgroundColor.Rgb !=
                                ExcelExtensions.BadColor.ToArgb().ToString())
        ...
        private void TryFillStreet(int row, ref string value)
        {
            ...
            //Поиск улиц
            var regs = new List<Regex> {streetToLeftRegex, streetRegex};
            foreach (var reg in regs)
            {
                ...
                if (match.Success)
                {
                    ...
                    if (streetCell.IsEmpty() || streetCell.Value.ToString() == string.Empty ||
                        streetCell.Value.ToString() != name &&
                        (typeOfStreetCell.IsEmpty() ||
                         typeOfStreetCell.Value.ToString().ToLower() == "микрорайон".ToLower()))
                    {
                        ...
                        else if (typeOfStreetCell.IsEmpty() && streetCell.IsEmpty() == false)
...
//
            ...
            var types = customTable.Rows.Cast<DataRow>()
                ...
                .OfType<string>()
                .Distinct().ToList();
            if (types.Count == 1 && nearCityCell.IsEmpty() == false)
            {
                if (typeOfNearCity.IsEmpty() ||
            ...
            //И мы уже имеем какой-то тип
            else if (typeOfNearCity.IsEmpty() == false)
                ...
                if (
                    ...
                    nearCityCell.IsEmpty() == false && //2. у нас есть насел пункт
                    customTable.Rows.Cast<DataRow>()
                        .Any( //3. В выборке есть комбинация текущий насел пункт + текущий тип
                            r =>
                                string.Equals(r[OKTMOWorksheet.Columns.NearCity - 1].ToString(),
            ...
            if (settlements.Count == 1)
            {
                if (settlementCell.IsEmpty() ||
                    !string.Equals(settlementCell.Value.ToString(), settlements.First(),
                        StringComparison.OrdinalIgnoreCase))
                {
                    if (settlementCell.IsEmpty() == false)
                ...
                else if (settlementCell.IsEmpty() == false &&
                         string.Equals(settlementCell.Value.ToString(), settlements.First(),
                             StringComparison.OrdinalIgnoreCase))
                {
                    settlementCell.Style.Fill.PatternType = ExcelFillStyle.Solid;
...
//                @"(?<=(?:^|\,|\.|(\())\s*)(?<name>(\b(?(?<=\()[а-яА-Я]|[а-яА-Я])[^\,\\\/\(\)\b\s\.]{3,}\b\s?){1,2})(?=\s*(?(\1)\)|(?:\.|\,|\(|$)))",RegexOptions.Compiled);
        private readonly Regex subjRegEx =
            new Regex(
                @"(?n)((?<pre>(?i)(по|в|за))\s)?(?<name>\b[А-Яа-я\-]+\b)\s(автономная(?=\sобл)\s)?(?<type>(?i)(?<=(ая|ой)\s)(обл(\.|(аст(ь|и))))|(?<=(ий|ый)\s)край)",RegexOptions.Compiled);

        private readonly Regex subjToLeftRegex =
            new Regex(
                @"(?n)((?<pre>(?i)(по|в|за))\s)?(?<type>(?i)респ(\.|ублик(а|е|и)))(?(?<=\.)\s?|\s+)(?<name>\b[А-Яа-я\-]+\b)",RegexOptions.Compiled);
